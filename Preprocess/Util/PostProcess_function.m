...
for spike_num = 1:length(UnitStrc)
    ...
    baseline = mean(raster_raw(:,(base_line_time)+pre_onset+1),2);
    highline1 = mean(raster_raw(:,(high_line_time1)+pre_onset+1),2);
    highline2 = mean(raster_raw(:,(high_line_time2)+pre_onset+1),2);
    ...
    if( min(p1,p2) < 0.001 && unitType(spike_num)~=0)
        ...
        [~,ww] = prune_wf(wf);
        GoodUnitStrc(good_idx).waveformBC = ww;

        wf = squeeze(template_ks(spike_num,:,:));
        [~,ww] = prune_wf(wf);
        GoodUnitStrc(good_idx).waveformKS = ww;
