...
param.duplicateSpikeWindow_s = 0.00004; % in seconds 
...
param.nRawSpikesToExtract = 500; % how many raw spikes to extract for each unit 
...
param.tauR_valuesMin = 1.2/1000; % refractory period time (s), usually 0.0020. 
...
param.tauR_valuesMax = 1.2/1000; % refractory period time (s), usually 0.0020
...
param.minThreshDetectPeaksTroughs = 0.3; % this is multiplied by the max value 
...
param.maxWvBaselineFraction = 0.4; % maximum absolute value in waveform baseline
    % should not exceed this fraction of the waveform's abolute peak value
param.maxScndPeakToTroughRatio_noise = 1; % peak must be less than this x the trough 
...
param.minTroughToPeak2Ratio_nonSomatic = 5 ; % trough should be min 3 x bigger than 1rst peak to count as non-somatic 
...
param.maxRPVviolations = 0.15; % fraction
param.maxPercSpikesMissing = 15; % in percentage
param.minNumSpikes = 300; % number of spikes
param.maxDrift = 100; % in micrometers 
param.minPresenceRatio = 0.8; % fraction
param.minSNR = 1;
