...
for aa = 1:length(interested_area)
    ...
    img = [];
    figure;
    set(gcf,'Position',[600 490 1350 195])
    ...
    tmp_data = [latency_save{1,1},latency_save{1,2}];
    xl_here = [110,240];
    axis equal;
    xlim(xl_here);ylim(xl_here)
    plot(xl_here,xl_here,Color=[0.5,0.5,0.5],LineWidth=1,LineStyle="--")
    ...
    pc_score ={};
    rsp_here = cv3_data(:,analysis_image);
    ...
    [coeff,pc_score{6},latent] = pca(rsp_here',NumComponents=5);

    data_here = [];
    for cc = 1:6
        for uu = 1:length(analysis_image)
            train_set = setdiff(1:length(analysis_image), uu);
            x = lscov([pc_score{cc}(train_set,:), ones([size(pc_score{cc},1)-1,1])],latency{1}(train_set)');
            predicted_latency(uu) = [pc_score{cc}(uu,:),1]*x;
        end
        data_here(cc) = corr(latency{1}',predicted_latency',type="Spearman");
    end
    subplot(1,6,6)
    bar(1:6, data_here,'EdgeAlpha',0,'FaceAlpha',0.5)
    xticklabels({'conv3','conv5','fc6','fc7','fc8','output'})
    ...
    subplot(1,6,3)
    imshow(img)
    imwrite(img, 'F3img.png')
    figsave(fullfile(root_dir,'Figs/F3/'),sprintf('F3E'))
end