...
for ses_idx = IT_session
    ...
    for uu = 1:length(good_units)
        ...
        r3 = squeeze(sum(PSTHData(uu_here,1:1000,general_info_data.global_params.pre_onset+(t1:t2)),3));
...
for ses_idx = IT_session
    ...
    for uu = 1:length(good_units)
        ...
        for tpoint = 1:length(decode_tin)
            ...
            tt_data_unit(tpoint,:) = zscore(PSTHData(uu_here,1:1000,general_info_data.global_params.pre_onset+tt_here));
        end
