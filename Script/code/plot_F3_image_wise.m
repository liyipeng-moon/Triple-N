...
PSTH_CombinedData = []; ses_number = []; avg_rsp = [];
for search_area = 1:height(manual_data)
    if(strcmp(manual_data.AREALABEL{search_area},interested_area))
        ...
        interested_unit = 1;
        sprintf('Area %d, have %d neuron, Cluster n=%d ',search_area,length(good_neuron_idx),sum(idx==interested_unit))
...
meta_example = load(fullfile(H5_dir,"ses10_240726_M5_3_info.mat"));
load(fullfile(root_dir,'Data','others','img_pool.mat'))
load(fullfile(prep_dir,'Clus_save.mat'),'all_cluster','clus_save')
...
load(fullfile(root_dir,'Data','others','ModelFeature','alexnet_resp.mat'))
...
subplot(2,3,3)
imshow(img)
mkdir(fullfile(root_dir,'Figs','F3A'))
figsave(fullfile(root_dir,'Figs','F3A'),sprintf('F3E'))